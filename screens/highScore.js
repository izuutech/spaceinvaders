...
export default function HighScore() {
  ...
  const sendScore = (values) => {
    ...
    fetch("https://spaceinvadersbackend.vercel.app/update", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        if (data.message) {
          asyncData
            .storeString("username", `${values.email.split("@")[0]}`)
            .then((valuee) => {
  ...
  useEffect(() => {
    fetch("https://spaceinvadersbackend.vercel.app/all")
      .then((res) => res.json())
      .then((data) => {
        if (data) {
          //now store global highscore to phone storage
          asyncData
            .storeString("globalHigh", `${data.message[0].score}`)
            .then((valuee) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.scoreText}>
        Current High Score: {playerHighScore}
      </Text>

      <TouchableOpacity onPress={openModal}>
        <View style={styles.update}>
          <Text style={styles.updateText}>Update Highscore</Text>
        </View>
      </TouchableOpacity>
      <Text style={styles.explainText}>
        By tapping the button above, you will be sending your score to the
        global server and updating it.
      </Text>
      ...
      <View style={styles.scoresContainer}>
        ...
        {allScores.map((score) => (
          <View key={score._id} style={styles.usersContainer}>
            <Text style={styles.eachScore}>{score.email.split("@")[0]}</Text>
            <Text style={styles.eachScore}>{score.score}</Text>
          </View>
        ))}
        {!allScores[0] && <Text style={styles.eachScore}>Loading...</Text>}
      </View>
      {modalOpen && (
